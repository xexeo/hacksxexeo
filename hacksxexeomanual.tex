...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item \verb!comentarios!, que permite comentários editoriais com os comandos |\hxcand{}|, |\hxcandr|, e outros.
            \item \verb!rascunhos!, que cria um ambiente marcado como rascunho.
            \item \verb!sugestoes!, que permite usar comandos que inserem |\hxcandsug|, removem |\hxcandrem| e trocam texto marcado |\hxcandtroca|.
...
\hxdefanoncitettext{[C]}
 ...
 \label{sec:com}   
    ...
    \begin{enumerate}
        \item \verb!\hxprof[texto a colorir]{comentário}!, pelo professor, ou
        \item \verb!\hxprofr[texto a colorir]{comentário}{rótulo}!, pelo professor, que permite dar um rótulo ao comentário, que deve ser único,
        \item \verb!\hxcand[texto a colorir]{comentário}!, pelo candidato.
        \item \verb!\hxcandr[texto a colorir]{comentário}{rótulo}!, pelo candidato, que permite dar um rótulo ao comentário, que deve ser único,
        \item \verb!\favorcitar[instrução adicional]!, que pede uma citação, possivelmente com instruções adicionais.
        \item |\hxprofx[texto a colorir]{comentário}| que mostra um comentário pelo professor \textit{striked-out}, para mostrar que foi tratado
        \item |\hxcandx[texto a colorir]{comentário}| que mostra um comentário pelo candidato \textit{striked-out}, para mostrar que foi tratado
        \item |\hxprofrx| e |\hxcandrx| que também mostram comentários com rótulos e \textit{striked-out}
    \end{enumerate}
    ...
    O primeiro exemplo é simples, apenas um comentário do professor\hxprof{Primeiro comentário é simples.}.
    O segundo comentário terá \hxprof[uma parte colorida]{O segundo exemplo é com uma parte marcada}.
    
    A marcação é suavizada em relação a cor definida. Atualmente o valor é de \makeatletter$\cor@suavizacao\%$\makeatother.
    
    Um comentário do candidato é similar professor\hxcand{Primeira comentário do candidato é simples.}. A segunda citação tera \hxcand[uma parte colorida]{O quarto exemplo é com uma parte marcada}.
    
    Finalmente, os rotulados, tanto para professor\hxprofr{Exemplo rotulado}{Rot1}, como para candidato\hxcandr{Olha a resposta}{Resposta para Rot1}. Que permitem também as versões \hxprofr[com marcação]{Exemplo rotulado 2}{Rot2}, para \hxcandr[ambos]{Olha a resposta 2}{Resposta para Rot2}.
    ...
    \xexeo[Para comprovar o funcionamento, mostramos que também é possível
    fazer uma marcação muito longa que inclui várias linhas e continua 
    funcionando.]{Marcação longa}
...
Este parágrafo \hxprofsug[Uma sugestão para inserir]{tem uma sugestão com comentário} e 
uma \hxprofsug{sem comentário, ficando só a marca no texto, e não indo para a lista de comentários}, usando o comando \verb*|\hxprofsug|, que só tem uma inserção de texto. Também é possível \hxprofrem[Aqui remove]{remover} ou \hxproftroca[trocando a palavra]{cambiar}{trocar.}
...
uma \xexeosug{sem comentário, ficando só a marca no texto, e não indo para a lista de comentários\label{sug:com1}}, usando o comando \verb*|\hxprofsug|, que só tem uma inserção de texto. Também é possível \xexeorem[Aqui remove]{remover} ou \xexeotroca[trocando a palavra]{cambiar}{trocar.}
...
ou um parágrafo 
\xexeorem{como esse a retirar}, também deviam ser comentadas dentro dele.
    ...
    \begin{verbatim}
        ...
        \begin{rascunho}
            Esse rascunho tem o título default.
        \end{rascunho}
    \end{verbatim}
    ...
    \begin{itemize}
        ...
        \item |\todo| e comentários como |\hxcand| e |\hxprof| não podem usar |verbatim| no texto.
